...
const CoopCard = ({bg, heading, image, redirect, value, isNotLikeVideo , setIsCoopMainClick, isCoopMainClick}) => {
  ...
  return (
    <div
    className={`block filter brightness-100 ${!isCoopMainClick && "bounce2" } max-w-sm p-6 ${bg} border border-gray-200 rounded-lg shadow hover:bg-gray-100 relative group transition-transform transform hover:scale-110 lg:min-w-[20%] w-[90%] lg:h-[220px] cursor-pointer`}
    onClick={() => {
      setIsCoopMainClick();
      if (!isCoopMainClick) {
        const cardsSection = document.getElementById('cardsSection1');
